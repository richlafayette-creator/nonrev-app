...
describe('employee verification access', () => {
  ...
  it('uses value-first verification copy and searchable airline selection', () => {
    ...
    assert.match(verifyPage, /t\('verifyHeadline'\)/)
    assert.match(verifyPage, /list="nonrevy-airline-employers"/)
    assert.match(verifyPage, /t\('companyEmailNotMapped'\)/)
    assert.match(homePage, /t\('homeHeadline'\)/)
    assert.match(conversationalHome, /t\('homeHeadline'\)/)
    assert.match(homePage, /t\('homeStepSearch'\)/)
    assert.match(conversationalHome, /t\('homeWorkflowBackups'\)|t\('homeStepBackups'\)/)
  })
})
