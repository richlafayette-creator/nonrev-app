...
describe('private beta membership and billing presentation', () => {
  it('renders one truthful Membership and Billing experience for membership and billing routes', () => {
    ...
    assert.match(membershipContent, /Membership & Billing/)
    assert.match(membershipContent, /Private beta access is currently complimentary\./)
    assert.match(membershipContent, /Payment details will be requested only when paid memberships launch\./)
    assert.match(membershipContent, /Current plan/)
    assert.match(membershipContent, /Private Beta/)
    assert.match(membershipContent, /Status/)
    assert.match(membershipContent, /Active/)
  })

  it('represents planned founding paid-beta prices accurately', () => {
    assert.match(membershipContent, /Core Founding Membership/)
    assert.match(membershipContent, /\$49\/year/)
    assert.match(membershipContent, /AI Travel Concierge Founding Membership/)
    assert.match(membershipContent, /\$99\/year/)
    assert.match(membershipContent, /Core 30-day Trip Pass/)
    assert.match(membershipContent, /\$14\.99/)
    assert.match(membershipContent, /Concierge 30-day Trip Pass/)
    assert.match(membershipContent, /\$24\.99/)
    assert.match(membershipContent, /monthly usage allowance/i)
