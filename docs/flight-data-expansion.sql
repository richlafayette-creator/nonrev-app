...
-- 1a) Required route freshness diagnostics columns.
-- Apply these when stored Supabase flight rows are used for beta route freshness checks.
-- `flight_date` enables requested-date versus stored-date warnings.
-- `source_checked_at` enables stale-ingestion warnings and is already listed above for provider audit metadata.
alter table public.flights add column if not exists flight_date date;
alter table public.flights add column if not exists source_checked_at timestamptz;
create index if not exists flights_flight_date_idx on public.flights (flight_date desc);
create index if not exists flights_source_checked_at_idx on public.flights (source_checked_at desc);
