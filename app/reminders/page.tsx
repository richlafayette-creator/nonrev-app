...
function responseColor(response: string) {
  if (response === 'Yes' || response === 'completed') return '#22c55e'
  if (response === 'Cancelled Trip' || response === 'dismissed') return '#facc15'
  if (response === 'No') return '#f87171'
  return '#38bdf8'
}
  ...
  return (
    <main className="app-shell" style={{ minHeight: '100vh', background: '#020617', color: 'white', padding: 40, fontFamily: 'Arial' }}>
      ...
      <section style={{ maxWidth: 1120, margin: '0 auto' }}>
        <p style={{ color: '#f472b6', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: 1 }}>
        ...
        <section className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14, margin: '24px 0' }}>
          {[
            ['Pending Reminders', reminderStats.pending, '#f472b6'],
            ['Completed Reminders', reminderStats.completed, '#22c55e'],
            ['Due Travel Items', dueCandidates.length, '#facc15'],
            ['Outcome Success Rate', `${outcomeStats.successRate}%`, '#38bdf8'],
            ['Outcome History Items', outcomeStats.outcomeCount, '#c084fc']
        ...
        <section style={{ border: '1px solid #334155', borderRadius: 22, padding: 20, background: '#0f172a', marginBottom: 24 }}>
          <strong style={{ color: '#34d399' }}>What updates when you respond?</strong>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {['Outcome History', 'Success Rate', 'Historical Route Database', 'Reputation System', 'Intelligence Dashboard'].map((label) => (
              <span key={label} style={{ border: '1px solid #334155', borderRadius: 999, padding: '8px 12px', color: '#cbd5e1', background: '#020617' }}>{label}</span>
            ))}
          </div>
        </section>
        ...
        <section style={{ marginTop: 24 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button type="button" onClick={refresh} style={{ padding: '10px 14px', borderRadius: 10, border: '1px solid #38bdf8', background: '#0f172a', color: '#bae6fd', fontWeight: 'bold' }}>
              Check for reminders
            </button>
          </div>
          ...
          <div style={{ display: 'grid', gap: 14, marginTop: 16 }}>
            ...
            {pendingReminders.map((reminder) => (
              <article key={reminder.id} className="flight-card" style={{ border: '1px solid #334155', borderRadius: 18, padding: 18, background: '#0f172a' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                  <div>
                    <strong style={{ color: '#f472b6', textTransform: 'uppercase', letterSpacing: 1 }}>{reminder.prompt}</strong>
                    <h3 style={{ margin: '8px 0' }}>{reminder.title}</h3>
                    <p style={{ color: '#38bdf8', fontWeight: 'bold', margin: '6px 0' }}>{reminder.route}</p>
                    <small style={{ color: '#94a3b8' }}>{reminder.carrier} · Travel date {reminder.travelDate} · {reminder.sourceType.replace('-', ' ')}</small>
                  </div>
                  <strong style={{ color: '#facc15' }}>{statusLabel(reminder)}</strong>
                </div>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 12 }}>
                  {(['Yes', 'No', 'Cancelled Trip'] as OutcomeReminderResponse[]).map((response) => (
                    <button
                      ...
                      style={{ padding: '10px 14px', borderRadius: 10, border: 'none', background: responseColor(response), color: response === 'Cancelled Trip' ? '#020617' : 'white', fontWeight: 'bold' }}
                    >
                      {response}
                    </button>
                  ))}
                  <button type="button" onClick={() => dismiss(reminder)} style={{ padding: '10px 14px', borderRadius: 10, border: '1px solid #475569', background: '#020617', color: '#cbd5e1', fontWeight: 'bold' }}>
                    Dismiss
                  </button>
                </div>
              </article>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14, marginTop: 16 }}>
            {candidates.length === 0 && (
              <article className="flight-card" style={{ border: '1px solid #334155', borderRadius: 18, padding: 18, background: '#0f172a' }}>
                ...
                <p style={{ color: '#cbd5e1', marginBottom: 0 }}>Only saved trips and watched itinerary options with real travel dates generate reminders.</p>
              </article>
            )}
            {candidates.map((candidate) => (
              <article key={`${candidate.sourceType}-${candidate.sourceId}`} className="flight-card" style={{ border: '1px solid #334155', borderRadius: 18, padding: 18, background: '#0f172a' }}>
                <strong style={{ color: candidate.due ? '#facc15' : '#38bdf8', textTransform: 'uppercase', letterSpacing: 1 }}>{candidate.due ? 'Due' : 'Scheduled'}</strong>
                <h3 style={{ margin: '8px 0' }}>{candidate.title}</h3>
                <p style={{ color: '#cbd5e1', margin: '6px 0' }}>{candidate.route}</p>
                <p style={{ color: '#94a3b8', marginBottom: 0 }}>{candidate.carrier} · {candidate.travelDate} · {candidate.reason}</p>
              </article>
            ))}
          </div>
        </section>
                ...
                <article key={reminder.id} className="flight-card" style={{ border: '1px solid #334155', borderRadius: 18, padding: 18, background: '#0f172a' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                    <div>
                      ...
                      <p style={{ color: '#38bdf8', fontWeight: 'bold', margin: '6px 0' }}>{reminder.route}</p>
                      <small style={{ color: '#94a3b8' }}>{reminder.carrier} · Travel date {reminder.travelDate}</small>
                    </div>
                    <strong style={{ color: responseColor(reminder.status) }}>{statusLabel(reminder)}</strong>
                  </div>
                  {reminder.notes && <p style={{ color: '#cbd5e1', marginBottom: 0 }}>{reminder.notes}</p>}
                </article>
              ))}
            </div>
          </section>
